...
define(function(require, exports, module){
	...
	$('.Y-SelectBranch').each(function(index){
		var province = $('.province')||$($(this).attr('province'));
		var city = $('.city')||$($(this).attr('city'));
		var branch = $('.branch')||$($(this).attr('branch'));
		...
		sel.provinceList.on('change',function(value,txt){
			sel.renderDiv.find('input.province').val(value);
		});
		sel.cityList.on('change',function(value,txt){
			sel.renderDiv.find('input.city').val(value);
		});
		sel.branchList.on('change',function(value,txt){
			sel.renderDiv.find('input.branch').val(value);
		});
        ...
        var startValue = {};
        if(province.val()) {
            startValue.province = province.val();
            startValue.city = city.val();
            startValue.branch = branch.val();
            sel.setValue(startValue);
        }
	})
});
})($);
