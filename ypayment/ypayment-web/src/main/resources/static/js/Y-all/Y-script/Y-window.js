	...
	Y.inherit('Window','container',{
		    ...
		    if(cfg.iframe === false) {
				...
				this.on('render',function(){
					...
					this.bind(frm,'load', function(){
					    frm.css({width: 'auto', height: 'auto'});
						_this.layout();
		...
		liveDrag: function(ele){//该函数激活拖拽
			...
			this.bind($(document),'mousemove',function(e){
				...
				_this.moveTo({x: rect.x + dx, y:rect.y + dy});
			    return false;
			});
        ...
        wndRect: function(){
            var wnd = this.wnd;
            var x = wnd.css('left'),y = wnd.css('top'),w = wnd.outerWidth(),h = wnd.outerHeight();
            var rect = {
                x: parseInt(x),y: parseInt(y),h: h,w: w
            };
            return rect;
        },
		...
		center: function(animate){
			...
			this.moveTo({x: x, y: y}, animate);
		},
		centerX: function(animate){
		    var x = ($(window).width() - this.wnd.width())/2 +($(document).scrollLeft() || 0);
			this.moveTo({x: x},animate);
		},
		centerY: function(animate){
		    var y = ($(window).height() - this.wnd.height())/2 +($(document).scrollTop() || 0);
			this.moveTo({y: y},animate);
		},
		moveTo: function(point,animate,callback){
			...
			if(animate) {
			    ...
			    this.wnd.animate({left: point.x, top: point.y}, speed, function(){
				    delete _this.moving;
					if(callback) callback();
				});
			} else {
			    this.wnd.css({left: point.x, top: point.y});
			}
			this.fire('move');
		},
		...
		countSize: function(){
			...
			var minWidth = cfg.minWidth, minHeight = cfg.minHeight;
            var width = itemWidth;
			...
			this.body.css({width: width, height: bodyHeight,'overflow-y':'auto','overflow-x':'auto'});
